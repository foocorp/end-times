...
{if ($logged_in)}
<li class="nav-item"><a class="edit-profile-link" href="/user/{$this_user->name}">{if ($this_user->getAvatar(24))}<img src="{$this_user->getAvatar(24)}" width="24" height="24" alt loading="lazy" title="Go to your profile"></a> &nbsp;{/if}<a class="edit-profile-link" title="Go to your profile" href="/user/{$this_user->name}">{$this_user->name}</a></li>
