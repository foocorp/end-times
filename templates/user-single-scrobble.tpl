...
<p>Listened to <span class="p-music-track"><a href="{$track->getURL()|escape:'html'}">{$track|escape:'html':'UTF-8'}</a></span> {if $artist}by <span class="p-music-artist"><a href="{$artist->getURL()|escape:'html'}">{$artist|escape:'html':'UTF-8'}</a></span>{/if}{if $album} on <span class="p-music-album">{$album|escape:'html':'UTF-8'}</span>{/if}</p>
