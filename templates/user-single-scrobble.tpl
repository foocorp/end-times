...
<p>Listened to <span class="p-music-track">{if $artist}<a href="/artist/{$artist|escape:'html'}/track/{$track|escape:'html'}">{$track}</a>{else}{$track}{/if}</span> {if $artist}by <span class="p-music-artist"><a href="/artist/{$artist|escape:'html'}">{$artist}</a></span>{/if}{if $album} on <span class="p-music-album">{$album}</span>{/if}</p>
