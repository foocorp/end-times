...
<p>Listened to <span class="p-music-track"><a href="/artist/{$artist|escape:'html'}/track/{$track|escape:'url'}">{$track|escape:'html':'UTF-8'}</a></span> {if $artist}by <span class="p-music-artist"><a href="/artist/{$artist|escape:'html'}">{$artist|escape:'html':'UTF-8'}</a></span>{/if}{if $album} on <span class="p-music-album">{$album|escape:'html':'UTF-8'}</span>{/if}</p>
